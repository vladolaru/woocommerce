# Native Payments B1h Multi-Currency Analytics SQL Projection Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use $subagent-driven-development (recommended) or $executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a non-mutating native projection for WooPayments-compatible multi-currency analytics SQL clauses.

**Architecture:** Keep analytics SQL ownership separate from hook registration. Add one pure service that accepts analytics clauses, context, storage mode, and sanitized currency args, then returns WooPayments-compatible SELECT, JOIN, WHERE, and selected-currency order SELECT clauses without querying the database or registering filters.

**Tech Stack:** PHP 8.1, WooCommerce internal services, PHPUnit, WordPress `$wpdb` table naming, WooCommerce HPOS order tables.

---

## File Structure

- Create `plugins/woocommerce/src/Internal/MultiCurrency/Services/MultiCurrencyAnalyticsSqlProjectionService.php`
    - Responsibility: project SQL clause arrays only.
    - No constructor dependencies, no hooks, no database reads, no asset registry writes.
- Create `plugins/woocommerce/tests/php/src/Internal/MultiCurrency/Services/MultiCurrencyAnalyticsSqlProjectionServiceTest.php`
    - Responsibility: input-output tests for legacy and HPOS clause projection.
- Create `plugins/woocommerce/changelog/add-native-payments-b1h-multi-currency-analytics-sql-projection`
    - WooCommerce changelog entry for the B1h native analytics SQL projection slice.

## Task 1: Write The Failing SQL Projection Tests
...
class MultiCurrencyAnalyticsSqlProjectionServiceTest extends WC_Unit_Test_Case {
	...
	public function test_projects_select_clauses_for_legacy_order_storage(): void {
		...
		$this->assertStringContainsString( "{$wpdb->prefix}wc_order_stats", implode( ' ', $result ) );
	}
	...
	public function test_projects_selected_currency_order_select_clauses(): void {
		...
		$result = $sut->project_selected_currency_order_select_clauses(
			array( "{$wpdb->prefix}wc_order_stats.net_total," )
		);

		$this->assertSame(
			"CASE WHEN wcpay_multicurrency_stripe_exchange_rate_meta.meta_value IS NOT NULL THEN ROUND({$wpdb->prefix}wc_order_stats.net_total / wcpay_multicurrency_stripe_exchange_rate_meta.meta_value, " . wc_get_price_decimals() . ") ELSE ROUND({$wpdb->prefix}wc_order_stats.net_total * wcpay_multicurrency_exchange_rate_meta.meta_value, " . wc_get_price_decimals() . ") END as net_total,",
			$result[0]
		);
	}
}
...
```bash
pnpm --filter='@woocommerce/plugin-woocommerce' test:php:env -- --filter MultiCurrencyAnalyticsSqlProjectionServiceTest
```

Expected: FAIL because `MultiCurrencyAnalyticsSqlProjectionService` is missing.

## Task 2: Implement The SQL Projection Service
	...
	public function project_where_clauses( array $clauses, array $currency_args, bool $is_hpos_enabled ): array {
		...
		if ( array() !== $currency_is ) {
			$clauses[] = $wpdb->prepare(
				"AND {$currency_field} IN (" . implode( ', ', array_fill( 0, count( $currency_is ), '%s' ) ) . ')',
				...$currency_is
			);
		}
		...
		if ( array() !== $currency_is_not ) {
			$clauses[] = $wpdb->prepare(
				"AND {$currency_field} NOT IN (" . implode( ', ', array_fill( 0, count( $currency_is_not ), '%s' ) ) . ')',
				...$currency_is_not
			);
		}
...
Significance: minor
Type: add
Comment: Add native multi-currency analytics SQL clause projections.
```

- [ ] **Step 2: Run the B0-B1h regression set**
...
- Stage only:
    - `plugins/woocommerce/src/Internal/MultiCurrency/Services/MultiCurrencyAnalyticsSqlProjectionService.php`
    - `plugins/woocommerce/tests/php/src/Internal/MultiCurrency/Services/MultiCurrencyAnalyticsSqlProjectionServiceTest.php`
    - `plugins/woocommerce/changelog/add-native-payments-b1h-multi-currency-analytics-sql-projection`
...
- [ ] **Step 2: Run PHPCS and changed PHP lint**
...
markdownlint .agents/scratchpad/sessions/2026-06-15-core-native-payments/plans/2026-06-16-core-native-payments-b1h.md .agents/scratchpad/sessions/2026-06-15-core-native-payments/implementation-log.md
```
...
```bash
git add plugins/woocommerce/src/Internal/MultiCurrency/Services/MultiCurrencyAnalyticsSqlProjectionService.php plugins/woocommerce/tests/php/src/Internal/MultiCurrency/Services/MultiCurrencyAnalyticsSqlProjectionServiceTest.php plugins/woocommerce/changelog/add-native-payments-b1h-multi-currency-analytics-sql-projection
git commit -m "feat(payments): add native payments B1h multi-currency analytics SQL projection"
```

## Self-Review

- Spec coverage: B1h covers analytics SELECT/JOIN/WHERE clause projection and selected-currency order SELECT conversion only.
- Explicit gaps: hook registration, report cache toggling, REST request gating, checking whether multi-currency orders exist, direct tracker order-count SQL, admin script enqueue, and `AssetDataRegistry` writes remain later slices.
- Performance review: this slice constructs query fragments only. Dynamic `IN` lists guard against empty input and use `$wpdb->prepare()` placeholders. Added joins match WooPayments compatibility and remain unregistered until a later ownership-gated slice.
