...
readonly RECONCILE_FIXTURE_STATE_CODE='$missing = new stdClass(); $state = get_option( "e2e_woopayments_native_provider_state", $missing ); if ( ! class_exists( "WooCommerce_WooPayments_Native_CI_Provider_Fixture" ) && $missing !== $state ) { fwrite( STDERR, "The WooPayments fixture lifecycle state is invalid.\n" ); exit( 1 ); } if ( class_exists( "WooCommerce_WooPayments_Native_CI_Provider_Fixture" ) ) { $fixture = WooCommerce_WooPayments_Native_CI_Provider_Fixture::registered_instance(); if ( ! $fixture && $missing !== $state ) { $fixture = new WooCommerce_WooPayments_Native_CI_Provider_Fixture(); } if ( $fixture ) { $fixture->reconcile_fixture_state_before_reinstall(); } }'
