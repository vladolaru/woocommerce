...
- `plugins/woocommerce/tests/php/src/Internal/Payments/Providers/WooPayments/WooPaymentsCutoverReconciliationJobTest.php::test_reconciliation_preserves_plugin_origin_multi_currency_orders`: actual cutover normalization preserves exact WooPayments 11.1.0-shaped USD and EUR order tuples.
...
- T.1 batch 6 narrowing (2026-09-25): `shopper/multi-currency.spec.ts` is retired from the browser suite (its EUR conversion and persistence claims already had PHPUnit/Jest owners; only its My Account "historical orders keep their stored money and WooPayments metadata when shopper currency changes" case had no lower-layer owner). That case's My Account claim is now NEW `MultiCurrencyOrderContextServiceTest::test_uses_order_currency_on_my_account_orders_list` (an order's own currency, not the shopper's selected one, governs its My Account row) plus existing `MultiCurrencyFrontendPricesControllerTest` and `MultiCurrencyFrontendCurrenciesControllerTest` coverage for the underlying money projection. This narrows the claim: no browser proves the real cutover, a real EUR selection, and a real hard reload together render the same stored money on the same page in one pass; each half is proven in isolation instead.
