...
| Contract item | Fixed value |
| --- | --- |
| Intended selector (superseded 2026-09-25, was: collects exactly the fourteen cases below) | `WCPAY_RUNTIME=native pnpm --dir plugins/woocommerce test:e2e:with-env woopayments-native --project=woopayments-native-provider --workers=1 --retries=0 --grep "@fidelity:card-decline-vocabulary"`, which now collects exactly the five `D-SI-*` cases from `plugins/woocommerce/tests/e2e/tests/woopayments-native/shopper/provider-fidelity-card-declines.spec.ts`. The nine `D-PI-*` checkout dispatches moved below the browser in T.1 batch 2 (see the note above); one provider-backed decline-and-retry smoke for the family survives at `shopper/provider-fidelity-card-recovery.spec.ts:136`, outside this selector's spec file. |
...
- Request shape (method, minor amount, currency, `return_url`) for every method, including `A5` Klarna's `requires_action` → pending/no-`_charge_id` outcome: `WooPaymentsProviderGatewayAdapterTest::test_charge_sends_split_redirect_method_request`, a data provider over alipay/affirm/afterpay_clearpay/bancontact/klarna. For alipay this is in addition to, not a replacement for, `A1`'s own unchanged `expectRequestedRedirect` assertion — see `A1`'s row below.
...
- `A1b` (Alipay on Blocks): payment-method-list presence is existing `WooPaymentsCheckoutBridgeTest.php:1104`; the order-received LPM logo/alt text is the new `WooPaymentsOrderSuccessPageTest::test_filters_alipay_lpm_payment_method_title_on_order_received_page`; request shape is the ADP row above.
...
| `DP2` win | Submit one physical-product evidence payload containing the provider's exact `winning_evidence` test value with `submit=true`. The same dispute becomes `under_review` and then `won`; exactly one closed-won effect applies and no update effect exists, because the platform does not forward `charge.dispute.updated`. See the 2026-08-20 correction. |
| `DP3` lose | Submit one physical-product evidence payload containing the provider's exact `losing_evidence` test value with `submit=true`. The same dispute becomes `under_review` and then `lost`; exactly one replay-safe capped local dispute-refund effect applies and no update effect exists, for the same platform reason as `DP2`. See the 2026-08-20 correction. |
...
**Consequence for the claim.** The update half of both rows is replaced by its negation plus a tripwire: each case now asserts that no update note exists and says why. If the platform ever starts forwarding `charge.dispute.updated`, the tripwire fails, and that failure is the signal to restore the original requirement rather than to weaken the case again. The closed-won, closed-lost, capped-refund, replay-safety and identity halves are unchanged and still binding.

**Accepted risk.** Until the platform forwards it, this family does not prove that a merchant sees any acknowledgement between submitting evidence and the dispute closing. It proves only that the submission reaches the provider and that the terminal outcome and its effects arrive.
...
| Row 42 selector | `WCPAY_RUNTIME=native pnpm --dir plugins/woocommerce exec playwright test --config=tests/e2e/envs/woopayments-native/playwright.config.ts --project=woopayments-native-provider --workers=1 --retries=0 --grep "@fidelity:mc-transaction-details" --list tests/e2e/tests/woopayments-native/shopper/provider-fidelity-multi-currency.spec.ts` collects exactly `M2`; the focused provider execution reuses M2's one checkout and adds only read-only merchant navigation after the graph converges. |
