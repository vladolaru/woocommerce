...
/**
 * The provider-backed order-edit refund smoke, against the native confirmation flow.
 *
 ...
 *
 * The client suite's original only asserted the rendered order status, so a
 * refund that failed to reach the provider was invisible. Assertions about
 * money here read the provider's charge, not just the screen.
 *
 * This smoke owns one paid order end to end. The other status decisions have
 * targeted PHP and Jest coverage instead of repeating provider fixtures.
 */
