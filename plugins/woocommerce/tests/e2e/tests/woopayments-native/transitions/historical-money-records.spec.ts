...
function submittedProtection( body: string | null ) {
	...
	if ( tokens.length > 1 ) {
		throw new Error(
			'Historical pay-for-order observed multiple CTP wire values.'
		);
	}
	if ( tokens.length === 0 ) {
...
async function readOrderNotes(
	...
	if ( ! Array.isArray( notes ) ) {
		throw new Error(
			'Historical pay-for-order requires an order-note array.'
		);
	}
	return notes;
}
...
async function readProduct(
	...
	if (
		...
		throw new Error(
			'Historical pay-for-order requires a product object.'
		);
	}
...
function orderTotalMinor( order: Record< string, unknown > ): number {
	...
	if ( ! /^\d+\.\d{2}$/.test( total ) ) {
		throw new Error(
			`Historical pay-for-order requires a USD total, received ${ total }.`
		);
	}
...
function failedOrderFields(
	...
	if (
		...
		lines[ 0 ] === null ||
		Number( ( lines[ 0 ] as Record< string, unknown > ).product_id ) !==
			productId ||
		Number( ( lines[ 0 ] as Record< string, unknown > ).quantity ) !== 1
	) {
		throw new Error(
			'Historical pay-for-order requires the observed 10.01 USD failed-order tuple.'
		);
	}
	return {
...
function observedRecoveryCurrency( currency: string ): 'USD' {
	if ( currency !== 'USD' ) {
		throw new Error(
			'Historical pay-for-order requires the observed USD recovery currency.'
		);
	}
	return currency;
}
...
function observedRecoveryStatus( status: string ): 'processing' | 'completed' {
	if ( status !== 'processing' && status !== 'completed' ) {
		throw new Error(
			'Historical pay-for-order requires an observed paid order status.'
		);
	}
	return status;
}
		...
		preCutover: {
				...
				session.withProviderWriteLocks(
					...
					async () => {
						session.requireApprovedProviderFixture(
							`historical-pay-for-order-ctp-${
								protectionTarget ? 'true' : 'false'
							}`
						);
						const allocation =
						...
						await withClassicCheckoutPage(
							...
							async ( checkout ) => {
								...
								state.product =
									await session.createOwnedProduct( '10.01', {
								...
								await browser.openClassicCheckout(
									checkout.classicCheckout.path
								);
								await browser.fillBillingDetails(
									session.runId
								);
								await browser.selectWooPaymentsCard();
								await browser.fillTestCard(
								...
								await session.withProviderSubmissionJournal(
									...
									async () => {
										const observed =
											await browser.observeSubmissionInterval(
												( activate ) =>
													session.performWrite(
														activate
													),
												async () => {
													if (
														...
														throw new Error(
															'Historical pay-for-order requires the bounded decline notice.'
														);
													}
										...
										if (
											observed.dispatch.requests
												.length !== 1
										) {
											throw new Error(
												'Historical pay-for-order decline must dispatch once.'
											);
										}
										if (
											observed.result.newOrderIds
												.length !== 1
										) {
											throw new Error(
												'Historical pay-for-order decline must create one order.'
											);
										}
										state.orderId =
											observed.result.newOrderIds[ 0 ];
										await session.setOrderRunId(
											state.orderId,
											session.runId
										);
										await convergeFailedPayment(
											session,
											state.orderId
										);
									}
			...
			readFailedFixture: async () => {
				...
				if (
					...
					throw new Error(
						'Historical pay-for-order requires a live customer pay link.'
					);
				}
				state.orderKey = orderKey;
				state.paymentUrl = paymentUrl;
				state.customerId = customerId;
				const failedPayment = await convergeFailedPayment(
					session,
					orderId
				);
				...
				const observedOrder = failedOrderFields(
					order,
					product.id,
					stockQuantity
				);
				const fixture = {
					...
					source: {
						...
						sourceCommit:
							'f85392666c9b543cd24dbbf903e0dbe4cb2c5cee',
					},
					allocation: requireValue(
						state.allocation,
						'the validated transition allocation'
					),
					...
					myAccountPayLink: {
						orderId,
						orderKeySha256: sha256( orderKey ),
						customerId,
						pathSha256: sha256( new URL( paymentUrl ).pathname ),
					},
					clientDecline: {
				...
				return {
					...fixture,
					checksumSha256: sha256( JSON.stringify( fixture ) ),
				};
			},
			readFailedPayment: () =>
				readFailedPaymentEvidence(
					session,
					requireValue( state.orderId, 'the failed order ID' )
				),
			cutOver: async () => {
		...
		postCutover: {
			...
			captureProtection: async () => {
				const scope = requireValue(
					state.scope,
					'the card-testing protection scope'
				);
				await scope.registerFreshContext( page );
				await page.goto(
					requireValue(
						state.paymentUrl,
						'the immutable order pay link'
					)
				);
				validateHistoricalOrderPayRoute( page.url(), {
					paymentUrl: requireValue(
						state.paymentUrl,
						'the immutable order pay link'
					),
					orderId: requireValue(
						state.orderId,
						'the immutable order ID'
					),
					orderKey: requireValue(
						state.orderKey,
						'the immutable order key'
					),
				} );
				...
				return scope.captureAuthenticatedSessionProtection(
					...
					requireValue(
						state.customerId,
						'the immutable order customer ID'
					)
				);
			},
			...
			submitPayForOrder: async () => {
				...
				expect( observed.result.orderId ).toBe(
					requireValue( state.orderId, 'the immutable order ID' )
				);
				expect( observed.result.orderKey ).toBe(
					requireValue( state.orderKey, 'the immutable order key' )
				);
				state.successOrderId = requireValue(
					state.orderId,
					'the immutable order ID'
				);
				return {
					requestCount: observed.dispatch.requests.length,
					submittedProtection: submittedProtection(
						observed.dispatch.requests[ 0 ]?.body ?? null
					),
				};
			},
			...
			readColdRecoveryEvidence: async () => {
				...
				const customerId = Number( order.customer_id );
				const orderKey = String( order.order_key );
				const declinedIntentId = requireValue(
					state.declinedIntentId,
					'the declined PaymentIntent ID'
				);
				const declinedPaymentMethodId = requireValue(
					state.declinedPaymentMethodId,
					'the declined PaymentMethod ID'
				);
				...
				if (
					...
					throw new Error(
						'Historical pay-for-order requires a cold customer/order read.'
					);
				}
				return {
					...
					order: {
						...
						keySha256: sha256( orderKey ),
						customerId,
						currency: observedRecoveryCurrency( evidence.currency ),
						totalMinor: evidence.amountMinor,
						status: observedRecoveryStatus( evidence.orderStatus ),
						noteCount: notes.length,
					},
					nativeSuccess: {
...
for ( const [ title, contractId, protectionTarget ] of [
	...
	test(
		...
		async ( { page, pilotRuntime } ) => {
			const evidence = await runHistoricalPayForOrder(
				pilotRuntime,
				page,
				protectionTarget
			);
			expect( evidence.order.status ).toMatch(
