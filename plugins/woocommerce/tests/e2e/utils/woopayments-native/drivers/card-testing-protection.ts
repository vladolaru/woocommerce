...
try {
	...
	if ( 'capture-state' === $operation ) {
		...
		wcpay_e2e_emit( array( 'rowsMatch' => wcpay_e2e_row_equal( $account_row, $snapshot['accountOption'] ) && wcpay_e2e_row_equal( $force_row, $snapshot['forceOption'] ), 'effectiveProtection' => (bool) ( $wrapper['data']['card_testing_protection_eligible'] ?? false ), 'pageAbsent' => 0 === count( wcpay_e2e_pages( $snapshot['classicCheckoutSlug'] ) ) && 0 === $marker_count, 'sessionAbsent' => $session_absent ) );
	} else { wcpay_e2e_fail(); }
