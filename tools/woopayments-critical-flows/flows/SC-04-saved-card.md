# SC-04 — Saved card → checkout (classic + Blocks) · HYBRID (D + A) · **KNOWN-FAIL (B2)**
...
- Compare ref vs target: same end-state shape. (Today: target diverges → FAIL.)
...
### Current native behavior (the regression — B2)
...
**Verdict today: FAIL (UX + functional) on target, PASS on reference.** Re-run after B2 fix; both layers + both stores must PASS.
