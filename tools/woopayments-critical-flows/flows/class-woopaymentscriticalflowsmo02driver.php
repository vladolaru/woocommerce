<?php
/**
 * Deterministic MO-02 authorizations-list and row-capture driver.
 *
...
/**
 * Read the Uncaptured-tab data source or capture its exact seeded row.
 */
final class WooPaymentsCriticalFlowsMo02Driver {

	private const STATE_SCHEMA   = 'woopayments_mo02_state.v1';
	private const CAPTURE_SCHEMA = 'woopayments_mo02_capture.v1';
	...
	public static function run( array $tool_args ): void {
		...
		$operation = (string) ( $tool_args[1] ?? '' );
		$order_id  = (int) ( $tool_args[2] ?? 0 );
		$argument  = (string) ( $tool_args[3] ?? '' );

		if ( 'snapshot' === $operation ) {
			self::snapshot( $store, $order_id, $argument );
			return;
		}

		if ( 'capture' === $operation ) {
			self::capture( $store, $order_id, $argument );
			return;
		}
		...
		self::emit_and_halt(
			array(
				...
				'error_message' => 'Operation must be snapshot or capture.',
			),
			3
		);
	}
	...
	/**
	 * Capture through the same row-level REST route used by the Uncaptured tab.
	 *
	 * @param string $store     Store role.
	 * @param int    $order_id  Order ID.
	 * @param string $intent_id PaymentIntent ID.
	 */
	private static function capture( string $store, int $order_id, string $intent_id ): void {
		$route   = sprintf( '/wc/v3/payments/orders/%d/capture_authorization', $order_id );
		$payload = array(
			'schema'        => self::CAPTURE_SCHEMA,
			'status'        => 'blocked',
			'store'         => $store,
		...
		if ( ! in_array( $store, array( 'ref', 'target' ), true ) || $order_id <= 0 || ! preg_match( '/^pi_[A-Za-z0-9_]+$/', $intent_id ) ) {
			$payload['error_code']    = 'invalid_capture_fixture';
			$payload['error_message'] = 'Capture requires a valid store, order, and PaymentIntent.';
			self::emit_and_halt( $payload, 3 );
		}

		$order = wc_get_order( $order_id );
		if ( ! $order instanceof WC_Order ) {
			$payload['error_code']    = 'missing_capture_order';
			$payload['error_message'] = 'The capture order could not be loaded.';
			self::emit_and_halt( $payload, 3 );
		}
		$payload['charge_id'] = (string) $order->get_meta( '_charge_id', true );
		if ( $intent_id !== (string) $order->get_meta( '_intent_id', true ) ) {
			$payload['error_code']    = 'capture_fixture_mismatch';
			$payload['error_message'] = 'The capture intent does not belong to the order.';
			self::emit_and_halt( $payload, 3 );
		}

		$request = new WP_REST_Request( 'POST', $route );
		$request->set_param( 'payment_intent_id', $intent_id );
		$response = rest_do_request( $request );
		if ( is_wp_error( $response ) ) {
			$payload['error_code']    = (string) $response->get_error_code();
			$payload['error_message'] = self::clean_message( $response->get_error_message() );
			self::emit_and_halt( $payload, 3 );
		}
		...
		if ( $status >= 200 && $status < 300 ) {
			...
			self::emit_and_halt( $payload, 0 );
		}

		$payload['error_code']    = self::response_error_code( $data );
		$payload['error_message'] = self::response_error_message( $data );
		$blocked                  = self::capture_transport_blocked( $status );
		$payload['status']        = $blocked ? 'blocked' : 'fail';
		self::emit_and_halt( $payload, $blocked ? 3 : 1 );
	}
	...
	/**
	 * Decide whether an HTTP response is an unavailable prerequisite, not a product verdict.
	 *
	 * @param int $status HTTP response status.
	 */
	private static function capture_transport_blocked( int $status ): bool {
		return $status < 400
			|| $status >= 500
			|| in_array( $status, array( 0, 401, 403, 404, 408, 423, 425, 429 ), true );
	}
	...
	/**
	 * Normalize the endpoint's numeric or ISO-8601 creation time to Unix seconds.
	 *
	...
	/**
	 * Extract a bounded error code from REST data.
	 *
	 * @param mixed $data REST response data.
	 */
	private static function response_error_code( $data ): string {
		return is_array( $data ) && isset( $data['code'] ) ? substr( sanitize_key( (string) $data['code'] ), 0, 100 ) : 'capture_request_failed';
	}

	/**
	 * Extract a bounded error message from REST data.
	 *
	 * @param mixed $data REST response data.
	 */
	private static function response_error_message( $data ): string {
		$message = is_array( $data ) && isset( $data['message'] ) ? (string) $data['message'] : 'The capture request failed.';
		return self::clean_message( $message );
	}

	/**
	 * Sanitize and bound one error message.
	 *
	...
	/**
	 * Emit one JSON record and stop WP-CLI with the requested code.
	 *
	 * @param array<string,mixed> $payload   Record.
	 * @param int                 $exit_code Exit code.
	 */
	private static function emit_and_halt( array $payload, int $exit_code ): void {
		self::emit( $payload );
		WP_CLI::halt( $exit_code );
	}
}

WooPaymentsCriticalFlowsMo02Driver::run( $args );
