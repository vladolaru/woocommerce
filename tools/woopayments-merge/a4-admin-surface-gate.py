...
PLUGIN_OPTIONAL_SPLIT_BASELINE_FILES = {
    ...
    result: dict[str, Any] = {
        ...
        "rest_controller": check_tokens(
            ...
            paths["rest_controller"],
            {
                ...
                "route-activate": "/payments/pm-promotions/(?P<id>[^/]+)/activate",
                "route-dismiss": "/payments/pm-promotions/(?P<id>[^/]+)/dismiss",
...
def capture_plugin_baseline(plugin_repo: Path, failures: list[str]) -> dict[str, Any]:
    ...
    for logical_name, relative_path in PLUGIN_OPTIONAL_SPLIT_BASELINE_FILES.items():
        path = plugin_repo / relative_path
        if not path.is_file():
            baseline[logical_name] = {
                "status": "not-published-in-reference",
                "path": relative_path,
            }
            continue
