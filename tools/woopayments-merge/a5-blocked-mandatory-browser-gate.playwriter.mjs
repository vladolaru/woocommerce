// A5f blocked mandatory native WooPayments cutover browser gate.
//
// Local runbook gate for the target store. It verifies that a mandatory
// cutover preflight blocker renders the product blocked notice and leaves the
// standalone WooPayments plugin active. Do not weaken this to mask bugs.
...
const evidencePath = env.A5_GATE_EVIDENCE_PATH || path.join( dataDir, 'a5f-blocked-mandatory-browser-gate.json' );
...
function writeEvidence( status = 'running' ) {
	const evidence = {
		gate: 'a5f-blocked-mandatory-browser-gate',
		status,
...
async function assertAuthenticatedWpAdmin( page ) {
	...
	if ( ! passed ) {
		throw new Error( 'A5f blocked mandatory browser gate is not authenticated as WP Admin on the target store.' );
	}
}

async function assertBlockedNotice( page ) {
	const deadline = Date.now() + 10000;
	let bodyText = '';

	do {
		bodyText = await page.locator( 'body' ).innerText( { timeout: 20000 } );
		const passed = bodyText.includes( 'WooPayments could not be disabled because native WooPayments is not ready' );
		if ( passed ) {
			recordCheck( 'blocked-notice', true, {
				finalUrl: page.url(),
				requiredTokens: [ 'WooPayments could not be disabled because native WooPayments is not ready' ],
			} );
			return;
		}
	...
	recordCheck( 'blocked-notice', false, {
		finalUrl: page.url(),
		bodyTextSample: bodyText.replace( /\s+/g, ' ' ).trim().slice( 0, 1000 ),
	} );
	throw new Error( 'Missing blocked mandatory native WooPayments notice.' );
}

async function assertWooPaymentsActiveOnPluginsPage( page ) {
	...
	const passed = rowsWithActiveDeactivate.length > 0;

	recordCheck( 'woopayments-plugin-active', passed, {
	...
	if ( ! passed ) {
		throw new Error( 'WooPayments did not remain active under a mandatory preflight blocker.' );
	}
}
...
try {
	...
	await assertBlockedNotice( page );
	await assertWooPaymentsActiveOnPluginsPage( page );

	if ( failedResponses.length > 0 || consoleIssues.length > 0 ) {
		throw new Error( `A5f blocked mandatory browser gate saw ${ failedResponses.length } failed responses and ${ consoleIssues.length } console issues.` );
	}
